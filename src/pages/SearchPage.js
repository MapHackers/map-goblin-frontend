...
import { searchRepository, searchUser } from '../store/search';
...
function SearchPage({ location, history }) {
  ...
  const getSearchData = useCallback(
    async (keyword) => {
      dispatch(searchRepository(keyword));
    ...
    <div>
      ...
      <div style={{ marginLeft: '200px', marginRight: '200px' }}>
        <div style={{ marginTop: '20px', marginRight: 'auto' }}>
          <h2>
            {' '}
            "{searchValue}" 키워드에 대한 검색 결과가{' '}
            {maps?.length + users?.length > 0
              ? maps?.length + users?.length + '개 있습니다.'
              : '없습니다.'}
          </h2>
        </div>
        <Tabs defaultActiveKey="1">
          ...
          >
            <div
              className="지도 검색 리스트"
              style={{ height: '700px', overflow: 'auto', marginRight: '100px' }}
            >
              <List
                ...
                renderItem={(item) => (
                  ...
                  >
                    <List.Item.Meta
                      ...
                      title={
                        <Link
                          to={`/${item.ownerId}/repositories/${item.name}`}
                          style={{ fontSize: '16px' }}
                        >
                      ...
                      description={
                        item.description ? highlightedText(item.description) : item.description
                      }
                    />
                    <b>Created by</b>
                    <Avatar
                      ...
                      src={
                        item.userProfile
                          ? Api.defaults.baseURL + '/files/' + item.userProfile
                          : 'NoProfile.png'
                      }
          ...
          >
            <div
              className="유저 검색 리스트"
              style={{ height: '700px', overflow: 'auto', marginRight: '50px' }}
            >
              <List
                grid={{
                  align: 'middle',
                  ...
                  <List.Item>
                    <Card
                      ...
                      actions={[
                        <div>
                          총 좋아요 수 <br />
                          <LikeOutlined key="like" style={{ marginRight: '5px' }} />
                          {item.likeCounts}
                        </div>,
                        <div>
                          총 조회 수 <br />
                          <EyeOutlined key="visit" style={{ marginRight: '5px' }} />
                          {item.visitCounts}
                        </div>,
                      ]}
                    >
                      <Meta title={highlightedText(item.name)} description={item.email} />
                      <Meta style={{ marginTop: '10px' }} title={item.description} />
                    </Card>
                  </List.Item>
                )}
              />
