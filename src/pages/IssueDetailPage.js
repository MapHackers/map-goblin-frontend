...
const IssueDetailPage = ({ match, location, history }) => {
  ...
  const { userId, repositoryName, issue } = match.params;
  ...
  const handleSubmit = async () => {
    ...
    await Api.post(location.pathname, dataToSubmit)
  ...
  const checkIssue = async () => {
    await Api.post(location.pathname + '/check')
      .then((response) => {
        history.push(`/${userId}/repositories/${repositoryName}`);
      })
      .catch((err) => err.response);
  };
  ...
  const onClickBack = () => {
    history.push(
      `/${match.params.userId}/repositories/${match.params.repositoryName}`
    );
  };
        ...
        <CommonLayout>
          <Row style={{ textAlign: 'left' }}>
            ...
            <Col span={14}>
              ...
              <RequestForm>
                <div style={{ marginTop: '20px' }}>
                  ...
                  <Avatar
                    src={
                      profile
                        ? `${Api.defaults.baseURL}/files/${profile}`
                        : `${Api.defaults.baseURL}/files/NoProfile.png`
                    }
                    size={20}
                    shape="circle"
              ...
              <Button style={{ marginLeft: '45%' }} type="primary" onClick={onClickBack}>
                뒤로가기
              </Button>
